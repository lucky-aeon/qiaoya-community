...
- 下行帧（服务端 → 前端）：
  ...
  - 被提及提醒（仅对被 @ 的用户下发）：
    ...
    { "type": "mention", "payload": {
        ...
        "messageId": "MID",
        "senderId": "UID",
        "mentionedUserId": "U_X",          // 当前连接用户
        "content": "@你 看一下这个问题……",     // 消息内容预览（可能截断）
        "senderName": "Alice",
    ...
    ```
    前端建议：
    - 若当前页面正处于该房间，定位/高亮 `messageId` 对应消息
    - 若不在该房间，可弹出轻提示或在“通知”角标+1；用户点击后导航：`/app/chat/rooms/{roomId}?anchorId={messageId}`
  - 错误：
    ```json
...
## 9. @ 提及与站内通知策略
- 在线于该房间：除广播 `message` 外，还会向被 @ 的用户定向发送 `mention` 事件，便于前端做 UI 强提醒
- 不在该房间：向被 @ 的用户发送一条“站内通知”（列表接口 `/api/user/notifications` 可见，类型 `CHAT_MENTION`）
- 站外邮件：当前未配置对应模板，不会发送邮件
