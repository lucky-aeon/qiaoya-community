...
自动交替（推荐）：脚本会自动识别当前活跃侧（读取 Nginx 配置的 down 标记），将新版本部署到“闲的一侧”，并切流过去。

```bash
IMAGE=... PROFILE=prod ENV_FILE=/etc/qiaoya.backend.prod.env \
UPSTREAM_SWITCH=nginx NGINX_CONF=/etc/nginx/conf.d/qiaoya.conf \
TARGET_SIDE=auto \
/www/project/qiaoya/deploy-qiaoya-bluegreen.sh
```

也可强制指定目标侧：`TARGET_SIDE=blue` 或 `TARGET_SIDE=green`。

脚本步骤：
1) 拉取镜像
2) 启动目标侧容器（blue→8520 或 green→8521）
...
回滚：
- 将 `UPSTREAM_SWITCH=nginx` 再次执行脚本（设置 `TARGET_SIDE` 为回切目标，或保留 `auto` 由脚本识别并切回），或手动编辑 Nginx 并 reload
